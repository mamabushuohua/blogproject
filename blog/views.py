...
class PostDetailView(DetailView):
    ...
    def get(self, request, *args ,**kwargs):
        # 复写get方法的目的是因为每当文章被访问一次，就得到文章阅读量+11
        # get方法返回的是一个HttpResponse实例
        # 之所以需要先调用父类的get方法，是因为只有当get方法调用后，
        # 才有self.object 属性，其值为Post模型实例，即被访问的文章post
    ...
    def get_object(self, queryset=None):
        #复写get_object 方法的目的是因为徐亚对post的body进行渲染
        post = super(PostDetailView, self).get_object(queryset=None)
        md = markdown.Markdown(post.body, extensions=['markdown.extensions.extra', 
...
class CategoryView(IndexView):
    ...
    def get_queryset(self):
        cate = get_object_or_404(Category, pk=self.kwargs.get('pk')) #kwargs是一个字典
